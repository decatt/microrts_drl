...
def process_states(states, selected_units, map_size):
    ...
    for state,selected_unit in zip(states,selected_units):
        if selected_unit == -1:
            ...
            linear_features.append(torch.zeros((13,)))
        else:
                ...
                for x in range(map_size):
                    if state[y][x][13] != 1:
                        ...
                        if d >0:
                            ...
                            d_feature = d_feature + (1/d)*feature
            ...
            nodes_features.append(torch.cat([x[1] for x in nodes_feature]))
            linear_features.append(d_feature)
    return torch.cat((torch.stack(linear_features),torch.stack(nodes_features)),dim=1)
...
class ActorCritic(nn.Module):
    def __init__(self) -> None:
        ...
        self.encoder = nn.Sequential(
            layer_init(nn.Linear(29*num_closest+13, 512), std=0.01),
            nn.ReLU(),
        ...
        self.value = nn.Sequential(
                layer_init(nn.Linear(29*num_closest+13, 256), std=0.01),
                nn.ReLU(),
    ...
    def get_distris(self,states):
        policy_network = self.encoder(states)
        ...
        return [type_distris,move_distris,harvest_distris,return_distris,produce_distris,produce_type_distris,attack_distris]
...
class Agent:
    ...
    def get_sample_actions(self,processed_states,units):
        distris = self.net.get_distris(processed_states)
