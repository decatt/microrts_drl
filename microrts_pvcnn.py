...
from utils import layer_init, calculate_gae, data_argument,MaskedCategorical
...
def extract_centered_regions(tensor, points, kernel_size):
...
def process_states(states, selected_units, kernel_size=7):
    states = torch.tensor(states)
    pv_state = extract_centered_regions(states, selected_units, kernel_size)
    return pv_state
...
class Agent:
    ...
    def sample_env(self, check=False): 
        ...
        while len(self.exps_list[0]) < self.num_steps:
            ...
            processed_states = process_states(self.obs, unit_list, kernel_size)
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        net = ActorCritic()
        if evaluate:
            net.load_state_dict(torch.load("model\ppo_model_TwoBasesWorkers12x12_1499.pkl"))
