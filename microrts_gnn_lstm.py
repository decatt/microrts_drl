...
def process_states(states, selected_units, map_size):
    ...
    for state,selected_unit in zip(states,selected_units):
        if selected_unit == -1:
            nodes_features.append(torch.zeros((1,29)))
        else:
            state_unit = state[:,:,15:21]
            player1 = state[:,:,11].unsqueeze(-1).expand_as(state_unit)
            palyer2 = state[:,:,12].unsqueeze(-1).expand_as(state_unit)
            state_resource = state[:,:,14:15]
            state_unit_p1 = player1*state_unit
            state_unit_p2 = palyer2*state_unit
            state_unit_all = torch.cat((state_resource,state_unit_p1,state_unit_p2),dim=2)
...
class ActorCritic(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.lstm = nn.LSTM(29, 128, batch_first=True)
    ...
    def forward(self, states):
        hidden,_ = self.lstm(states)
        hidden,_ = pad_packed_sequence(hidden, batch_first=True)
        hidden = self.relu(self.fc(hidden[:, -1, :]))
        hidden = self.fc(hidden)
        hidden = self.relu(hidden)
        type_distris = MaskedCategorical(self.policy_type(hidden))
        move_distris = MaskedCategorical(self.policy_move(hidden))
        harvest_distris = MaskedCategorical(self.policy_harvest(hidden))
        return_distris = MaskedCategorical(self.policy_return(hidden))
        produce_distris = MaskedCategorical(self.policy_produce(hidden))
        produce_type_distris = MaskedCategorical(self.policy_produce_type(hidden))
        attack_distris = MaskedCategorical(self.policy_attack(hidden))
...
class Agent:
    ...
    def get_sample_actions(self,processed_states,units):
        data = pad_sequence(processed_states, batch_first=True)
        lengths = [len(processed_state) for processed_state in processed_states]
        ...
        distris,_ = self.net(packed_data)
        ...
        for i in range(len(action_mask_list)):
            action_mask_list[i][26] = 0
            action_mask_list[i][27] = 0
        ...
        actions = actions.T.cpu().numpy()
        actions = actions[original_idx]
        return actions, masks.cpu().numpy(),log_probs.T.cpu().numpy()
...
class Calculator:
    ...
    def get_prob_entropy_value(self,states, actions, masks):
        data = pad_sequence(states, batch_first=True)
        lengths = [len(processed_state) for processed_state in states]
        lengths, sorted_idx = torch.tensor(lengths).sort(descending=True)
        data = data[sorted_idx]
        _, original_idx = sorted_idx.sort()
        packed_data = pack_padded_sequence(data, lengths, batch_first=True)
    ...
    for i in range(10):
        ...
        for version in range(MAX_VERSION):
            ...
            if not evaluate:
                ...
                if (version+1) % 500 == 0:
                    torch.save(net.state_dict(), "model/ppo_model_"+map_name+"_"+str(version)+".pkl")
                    torch.save(net, "model/ppo_model_"+map_name+"_"+str(version)+".pth")
