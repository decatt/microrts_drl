...
class Agent:
    ...
    def sample_env(self, check=False):  
        ...
        while len(self.exps_list[0]) < self.num_steps:
            ...
            states = np.zeros((self.num_envs, 24, 24, 27))
            pad_w = (24 - w) // 2
            pad_h = (24 - h) // 2
            states[:, pad_w:pad_w + w, pad_h:pad_h + h, :] = self.obs
            action,mask,log_prob=self.get_sample_actions(states, unit_mask)
            ...
            for i in range(self.num_envs):
                ...
                self.exps_list[i].append([states[i],action[i],rs[i],mask[i],done,log_prob[i]])
                if check:
                    if done_n[i]:
